...
내부적으로 일부 엔드포인트는 `account_script/ctrader.py` worker (`http://127.0.0.1:8855`) 를 호출. worker 자체의 protobuf 엔드포인트 명세는 [`./endpoint_ctrader_worker.md`](./endpoint_ctrader_worker.md) 참조.
...
각 엔드포인트의 query 파라미터 / 응답 schema 는 [`./endpoint_ctrader_worker.md`](./endpoint_ctrader_worker.md) 의 동일 path 섹션 참조.
