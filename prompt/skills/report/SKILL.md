---
name: report
description: "Write a report to a file, or report directly in chat with the chat option. Use when 'write a report' or '보고서를 작성해'. When explicitly calling a skill by specifying 'rpt', or 'report', the skill must be used. (ex: ;isk rpt / ;rpt)"
argument-hint: "[format] [filename (except chat)] [instructions]"
---
...
- Save the report in the current path (current working directory), except when the `chat` or Artifact format rules below apply.
...
For file output only: if a report already exists, content can be added/modified/deleted in the existing report.
...
For formats other than `chat`, the order of the first two options is fixed. (First: report format, Second: report filename)
Any remaining arguments beyond the first two are interpreted as additional instructions for the report content.
For `chat`, there is no filename argument; all arguments after `chat` are additional instructions for the report content.
...
The `chat` option is also supported for reporting directly in the chat window without saving a file.
...
If the first argument is `chat`, summarize the conversation and present the full report directly in the chat response.
- Do not create, modify, or save report files, including temporary files. Do not publish an Artifact.
- This overrides the file-saving, existing-report editing/overwrite confirmation, filename, and extension rules. Do not ask for a filename.
- Use readable Markdown in the chat response and include the last updated time obtained with the `date` command specified above.
- Treat every argument after `chat` as an additional instruction for the report content, not as a filename.

Examples:
- `;rpt chat` -> Summarize the conversation and report only in chat.
- `;rpt chat Focus on today's analysis` -> Report only in chat, focusing on today's analysis.
...
This option does not apply to `chat`; arguments after `chat` are additional content instructions.
