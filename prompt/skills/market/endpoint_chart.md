...
`src/endpoint_tradingview.py` 기반 HTTP API 문서.

> 1 ~ 6 번 엔드포인트의 모든 응답(성공/실패)에는 KST 기준 현재 서버 시각이 `servertime_kst` 필드로 포함된다.
> JSON 응답에서는 항상 **최상단 첫 번째 키**로 위치하며, 포맷은 `"yyyy-mm-dd HH:MM:SS (KST +09:00)"` 이다.
> 예: `"2026-05-08 17:30:00 (KST +09:00)"`
...
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "DB 조회 실패" }
```
...
- 최상위 key 중 `"servertime_kst"` 는 KST 기준 현재 서버 시각 (항상 첫 번째 키), `"description"` 은 설명 필드, 나머지 key 는 모두 `"symbol:exchange"` 포맷.
...
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "query 파라미터가 필요합니다." }
```
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "TradingView HTTPError: ... | URLError: ... | JSON 파싱 실패: ..." }
```
...
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "symbol, exchange, interval, count 파라미터가 필요합니다." }
```
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "count는 정수여야 합니다." }
```
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "데이터를 찾을 수 없습니다. symbol=..., exchange=..., interval=..." }
```
```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "status": "error", "message": "DB 조회 실패" }
```
...
```
<servertime_kst>,<symbol>,<exchange>,<interval>,<rowCount>
...
- 1행: 메타데이터 (`servertime_kst,symbol,exchange,interval,row개수`)
  - `servertime_kst` 는 KST 기준 현재 서버 시각, 포맷은 `"yyyy-mm-dd HH:MM:SS (KST +09:00)"`. 항상 메타라인의 첫 컬럼.
  - 첫 컬럼 자체에 공백과 괄호가 포함되지만 콤마(`,`)는 들어있지 않으므로 CSV 파싱에는 영향이 없다.
  - 예: `2026-05-08 17:30:00 (KST +09:00),NQ1!,CME_MINI,1h,1000`
- 2행: 컬럼 헤더
...
`/tradingview/detail` 과 동일한 에러 응답 포맷 (`servertime_kst` 가 최상단 첫 키).
...
- 단일 행이 반환되더라도 최상위 구조는 항상 `{"servertime_kst": "...", "list": [ ... ]}` 포맷으로 유지된다.
...
> 400 응답은 plain text 이며 `servertime_kst` 가 포함되지 않는다. (한쪽만 입력된 비정상 요청에 대한 즉시 거절)

#### Schema (실패, 500)

```json
{ "servertime_kst": "yyyy-mm-dd HH:MM:SS (KST +09:00)", "list": [], "error": "DB 조회 실패" }
```
